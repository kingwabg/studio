...
- **캔버스 모드** (InputHandler.canvasMode + canvasEditingRef, 메뉴바 토글·기본 ON·localStorage):
  ...
  **표는 예외 — 글 넣는 그릇**: 셀 클릭=글영역 편집, 드래그=셀 범위 선택. 표 전체 개체 잡기 =
  ①**표 hover→흰 네모 핸들**(클릭=선택, e/s/se 드래그=리사이즈) ②표 테두리 클릭 ③Alt+클릭.
...
- **디자인 구현은 정본 대조 → 카탈로그 동기화** (2026-07-12 신설, 사용자 제안): 디자인 값의
  **정본 = `design/tokens.md`** (팔레트·사용처 지도·표면별 크기 번역·알려진 표류). UI를 새로
  만들거나 룩을 바꿀 때 ①구현 전 design/tokens.md + design/*.html 카드에서 유사 컴포넌트 확인
  — 있으면 그 스펙 재사용, 새로 짓지 말 것. 색을 지어내지 말고 표에 없으면 표부터 추가.
  ②구현 후 값 변경이 있으면 tokens.md 먼저 갱신 → 사용처 전파, 새/변경 컴포넌트는 카드에 반영
  후 DesignSync로 동기화(도구 가능 환경 — claude.ai/design "Document Studio 디자인 시스템").
  ③DesignSync 없는 환경(CLI 등)은 카드 파일만 갱신하고 보고에 "카탈로그 미러 미동기화" 명시.
...
- **검증을 통과했으면 확인 없이 커밋한다** (사용자 지시 2026-08-05 — 종전 "물어보고 커밋"에서
  전환. sc- 저장소는 이미 같은 규약이었다). 조건은 **완료 게이트 통과**다: 포크 수정이면
  `npx tsc --noEmit` + `npm test`, 내보내기 코어면 `verify:hwpx` — 통과 전에는 커밋하지 않는다.
  ⚠ 여전히 물어야 하는 것: **되돌리기 어려운 일**(브랜치 강제 push·이력 재작성·대량 삭제)과
  남의 미커밋 변경을 건드리는 경우. 내 변경만 골라 담는다(작업 트리에 남의 WIP가 섞여 있다).
  비밀값(.env.local·키)은 어떤 경우에도 커밋 금지 — 예시 파일(.env.example)에 실키를 넣지 말 것
  (실사고 2026-07-12 회수).
