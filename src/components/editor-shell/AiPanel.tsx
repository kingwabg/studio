  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 px-3.5 py-3.5">
      <div className="flex items-center gap-1.5 mb-2.5">
        <span className="text-accent flex"><IcSparkles size={15} /></span>
        <span className="text-[12.5px] font-semibold text-ink">AI 문서 도우미</span>
        {activeModel && (
          <span
            title={`현재 응답 모델: ${activeModel}`}
            className={`ml-auto text-[9px] font-semibold rounded px-1.5 py-0.5 border ${
              activeModel === "claude-fable-5"
                ? "text-accent bg-accentsoft border-accentline"
                : "text-inkfaint bg-paper border-line"
            }`}
          >
