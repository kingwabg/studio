...
| 음성 인식(녹음→회의록 v1) | **브라우저 내장 Web Speech + MediaRecorder** (2026-07-12) — 라이브러리·API 키 0 | 실측: SpeechRecognition ko-KR·연속·interim 전부 가용. STT 품질 경쟁은 우리 싸움터가 아님 — 차별점은 HWPX 완성. ⚠ 크롬이 음성을 구글 서버로 전송(동의 카드 필수)·화자 분리 없음 | v1 도그푸딩에서 인식률/장시간 안정성이 아프면 → 한국어 특화 유료 STT(화자 분리 포함)를 그 시점 최신 비교로 (P1-9 v2) |
