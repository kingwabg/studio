...
- **캔버스 모드 ② 새 문서 기본 여백 0 (P0-1)** ✅ (2026-07-12, 병렬 체제 첫 배치 A구역).
  main.ts createNewDocument에서 캔버스 모드(localStorage CANVAS_MODE_KEY≠'0')면 setPageDef로
  여백 7종 전부 0 — 화면 좌표=종이 원점=내보내기 봉투 원점. 문서 모드 새 문서는 기존 여백 유지.
  **⭐ 실측**: before 좌우30·상20·하15·머리/꼬리15mm → after 전부 0 / 좌표 일치: (50,50)mm
  더블클릭 텍스트 → getShapeProperties 오프셋 (50, 49.7)mm / 문서 모드 새 문서 30/30/20/15
  유지(동작 보존). tsc 0 · 185/185. ⚠ 검증 함정: create-new-document는 dirty면 미저장 가드
  다이얼로그에 막힘 — 직후 getPageDef는 기존 문서를 읽는다(재실험으로 판별).
