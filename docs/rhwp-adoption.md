...
### 외부 .hwp 서식 보존 실측 (2026-08-04) — 재측정 금지

관공서·재단 공모 서식을 **우리 엔진으로 열고 고쳐도 구조가 남는가**를 실물로 쟀다.
표본: 2025 바보의나눔 배분사업 사업계획서(.hwp 32,768B, 사용자 제공).

| | 원본 | 열고 그대로 저장 | **빈칸 6곳 채우고 저장** |
|---|---|---|---|
| 페이지 | 6 | 6 | 6 |
| 표 | 5 (25×12·24×14·35×25·38×18·5×4) | 동일 | **동일** |
| 채워진 셀 | 1,805 | 1,805 | **1,811** (+6) |
| 셀 글자 | 6,136 | 6,136 | **6,213** (+77) |

+77 은 써넣은 6개 문자열의 글자 수 합계와 **정확히 일치**한다(우연 일치가 아님을 검산).
hwp·hwpx 양쪽 저장 모두 동일. 원래 있던 텍스트 손실 0건. 체크박스(□)·공백 배치도 남는다.
→ **결론: 남의 서식을 열어 칸을 채우는 경로는 안전하다.** 다만 셀 병합·테두리·글꼴은
   아직 안 쟀고, 한컴 육안 확인은 사람 몫이다.

⚠ **측정 함정**: `getTextInCell` 은 (행, 열)이 아니라
`(sec, para, controlIdx, **셀 순번**, cellPara, offset, count)` 다. 행·열로 부르면 빈 문자열이
돌아오는데, 원본·결과 양쪽이 다 비어 "같으니 보존됨"으로 **거짓 통과**가 난다(실사고 2026-08-04).
표본이 비면 통과가 아니라 미검증이다.
