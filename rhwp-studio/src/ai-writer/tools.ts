...
export const WRITER_PROMPT =
  ...
  '⑤-1 수치 정책: 사용자가 준 수치만 사실로 쓴다. 없으면 [○○명] 로 남기는 것이 기본이나, ' +
  '사용자가 "예시로 채워줘"라고 하면 **일관된 가상 수치**로 채우되 done 보고에 "수치는 예시임"을 명시한다.\n' +
  ...
  '⑧-1 **밀도가 완성도다**: 실무 보고서는 섹션마다 문단+목록+표를 조합한다. 표에는 합계 행과 ' +
  '비율(%) 열을 넣고, 일정표에는 담당 열을 넣는다. 섹션 하나가 목록 두 줄로 끝나면 하급 문서다.\n' +
  '⑧-2 보고서·계획서는 **첫 섹션 앞에 문서 정보 표**(2열: 기관명/부서/작성일, 값 모르면 [기관명])를, ' +
  '**마지막에 맺음 문단**("이상과 같이 ○○을 보고함")을 넣는 관행을 따른다.\n' +
