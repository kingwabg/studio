...
// 격자 읽기(병합 안전)는 의존성 0 모듈로 추출 — 에이전트와 공유(중복 2회 룰). 재export 로 기존 사용처 유지.
import { readTable, blankCells, gridToPrompt, type GridCell, type TableGrid, type TableGridWasm } from './table-grid.ts';
export { readTable, blankCells, gridToPrompt, type GridCell, type TableGrid } from './table-grid.ts';

export interface Fill { table: number; row: number; col: number; cellIdx: number; text: string }

interface TableWasm extends TableGridWasm {
