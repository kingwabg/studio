...
export const SPELL_RULES: SpellRule[] = [
  ...
  // ⚠ '-는대' 는 인용 축약("먹는대")이 정상이라 통째로 규칙화할 수 없다.
  //   아래 둘은 인용형이 따로 있어("된대"·"그렇대") **항상 틀린** 형태만 골랐다.
  { re: /되는대(?!로)/g, msg: "'되는데'가 맞습니다", fix: '되는데' },
  { re: /그런대(?![요로])/g, msg: "'그런데'가 맞습니다", fix: '그런데' },
  { re: /되어든/g, msg: "'되든'이 맞습니다", fix: '되든' },
