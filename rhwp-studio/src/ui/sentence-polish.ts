...
/**
 * ⚠ "〔사람1〕 같은 표시는 그대로 두세요" 라고만 적었더니 모델이 **없던 〔사람1〕을
 *   지어냈다**(2026-08-01 실측). 조건을 명시해야 한다 — "있으면 그대로, 없으면 만들지 마라".
 * ⚠ "뜻은 그대로 두고" 만으로는 오타를 안 고친다("박으로"를 그대로 뒀다).
 *   오타가 많다는 전제와 "뜻을 추측해 고치라"는 지시를 함께 준다.
 */
const SYSTEM =
  '당신은 한국어 교정 편집자입니다. 사용자가 준 문단은 오타·띄어쓰기 오류가 많습니다. ' +
  '뜻을 추측해 자연스러운 한국어로 고쳐 **서로 다른 3가지**로 제시하세요.\n' +
  '출력은 JSON 하나만: {"versions":["...","...","..."]}\n' +
  '규칙: ①없는 내용을 새로 지어내지 마세요 ' +
  '②원문에 〔사람1〕 같은 표시가 있으면 그대로 두되, 없으면 절대 만들지 마세요 ' +
