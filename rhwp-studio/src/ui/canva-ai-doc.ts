...
/**
 * 글상자(=1×1 무테두리 표) 또는 표의 한 셀을 가리키는 wasm 접근 좌표.
 * cellIdx: 표 컨트롤 안 셀 인덱스(0부터). 글상자는 셀이 1개라 0(생략 시 0).
 */
export interface ShapeRef { sec: number; ppi: number; ci: number; cellIdx?: number; }
...
export interface DocTextElement {
  ...
  /** 현재 전체 텍스트 (문단들을 \n으로 결합) */
  text: string;
  /** 위치 맥락 라벨 (예: "표 2행 3열", "글상자") — findings에서 어디인지 표시용. 선택. */
  context?: string;
}
...
/** 글상자/표셀 전체 텍스트 읽기 (문단들 → \n 결합). cellIdx는 ref에서(글상자=0). */
export function readShapeText(wasm: any, ref: ShapeRef): string {
  const cell = ref.cellIdx ?? 0;
  const n = wasm.getCellParagraphCount(ref.sec, ref.ppi, ref.ci, cell);
  const lines: string[] = [];
  for (let cpi = 0; cpi < n; cpi++) {
    const len = wasm.getCellParagraphLength(ref.sec, ref.ppi, ref.ci, cell, cpi);
    lines.push(len > 0 ? wasm.getTextInCell(ref.sec, ref.ppi, ref.ci, cell, cpi, 0, len) : '');
  }
  return lines.join('\n');
}

/** 글상자/표셀 내용을 새 텍스트로 통째 교체 (스냅샷 안에서 호출). cellIdx는 ref에서. */
export function replaceShapeText(wasm: any, ref: ShapeRef, next: string): void {
  ...
  for (let cpi = n - 1; cpi >= 1; cpi--) {
    const len = wasm.getCellParagraphLength(ref.sec, ref.ppi, ref.ci, cell, cpi);
    if (len > 0) wasm.deleteTextInCell(ref.sec, ref.ppi, ref.ci, cell, cpi, 0, len);
    wasm.mergeParagraphInCell(ref.sec, ref.ppi, ref.ci, cell, cpi);
  }
  const len0 = wasm.getCellParagraphLength(ref.sec, ref.ppi, ref.ci, cell, 0);
  if (len0 > 0) wasm.deleteTextInCell(ref.sec, ref.ppi, ref.ci, cell, 0, 0, len0);
  // 새 텍스트 삽입 (줄마다 insert → 분할)
  const lines = next.split(/\r?\n/);
  for (let k = 0; k < lines.length; k++) {
    if (lines[k]) wasm.insertTextInCell(ref.sec, ref.ppi, ref.ci, cell, k, 0, lines[k]);
    if (k < lines.length - 1) wasm.splitParagraphInCell(ref.sec, ref.ppi, ref.ci, cell, k, lines[k].length);
  }
}
