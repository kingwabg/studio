...
async function initialize(): Promise<void> {
  ...
  try {
    ...
    // [캔버스 한컴 포크] 한컴독스처럼 시작하자마자 빈 A4 문서가 떠 있게 —
    // ?url 자동 로드·자동저장 복구를 먼저 기다리고, 그래도 빈 화면이면 새 문서 생성.
    void (async () => {
      await loadFromUrlParam();
      await offerAutosaveRecoveryIfIdle();
      if (wasm.pageCount === 0 && !documentState.isDirty()) {
    ...
    if (import.meta.env.DEV) {
      ...
      void import('@/lint/dict').then((m) => { (window as any).__dict = m; }); // 사전 e2e 판정용
    ...
    mountCanvaSidebars({ wasm, eventBus, dispatcher, getInputHandler: () => inputHandler });
  } catch (error) {
...
async function initializeDocument(docInfo: DocumentInfo, displayName: string): Promise<void> {
  ...
  try {
    ...
    // [캔버스 한컴 포크] 문서 준비 완료를 UI(캔바 인스펙터 등)에 알림 —
    // 새 문서/열기 직후엔 dirty 이벤트가 없어 컨텍스트 배너가 갱신되지 않는다.
    eventBus.emit('command-state-changed');
  } catch (error) {
