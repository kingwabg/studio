...
export function onCompositionStart(this: any): void {
  // [캔버스 한컴 포크] 캔버스 모드: 편집 컨텍스트 없이는 IME 조합도 시작하지 않는다
...
export function onInput(this: any, e?: InputEvent): void {
  ...
  // [캔버스 한컴 포크] 캔버스 모드: 편집 컨텍스트(재클릭/더블클릭 진입) 없이는 타이핑 무시 —
  // 캔바처럼 빈 캔버스 상태에서 키를 눌러도 본문이 오염되지 않는다.
  if (this.canvasMode && !this.canvasEditingRef
      && !this.cursor.isInHeaderFooter() && !this.cursor.isInFootnote()) {
    this.textarea.value = '';
    return;
  ...
  this.applyPendingToInserted?.(insertPos, text.length);  // 대기 서식 (일반 타이핑 경로)
