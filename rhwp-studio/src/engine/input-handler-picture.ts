...
export function updatePictureMoveDrag(this: any, e: MouseEvent): void {
  ...
  // [캔버스 한컴 포크] 캔바식 자석 스냅 — 시작점 기준 절대 델타에 스냅 보정을 얹고,
  // 프레임 적용분은 "목표 누적 − 지금까지 누적"으로 환산(기존 증분 방식과 결과 동일).
  // Alt = 스냅 해제(자유 이동). 스냅 컨텍스트는 드래그당 1회 수집(레이아웃 호출 절약).
  const rawDx = px - state.startPageX;
  const rawDy = py - state.startPageY;
  let snapDx = 0;
  let snapDy = 0;
  let guides: import('./canvas-snap').SnapGuide[] = [];
  if (!e.altKey) {
    if (state.snapCtx === undefined) {
      try {
        const moving = (state.multiRefs || [state.ref]).map((r: any) => ({ ppi: r.ppi, ci: r.ci }));
        const basePi = state.pageIndex ?? pi;
        const pageWpx = this.virtualScroll.getPageWidth(basePi) / zoom;
        const pageHpx = this.virtualScroll.getPageHeight(basePi) / zoom;
        state.snapCtx = buildSnapContext(this.wasm, basePi, moving, pageWpx, pageHpx) as SnapContext | null;
      } catch {
        state.snapCtx = null;
      }
    }
    if (state.snapCtx) {
      const b = state.snapCtx.bbox;
      const r = computeSnap({ x: b.x + rawDx, y: b.y + rawDy, w: b.w, h: b.h }, state.snapCtx.targets);
      snapDx = r.dx;
      snapDy = r.dy;
      guides = r.guides;
    }
  ...
  const basePi = state.pageIndex ?? pi;
  const layer = snapLayerFor(this.container);
  if (guides.length) {
    layer.show(guides, {
      zoom,
      pageLeft: this.virtualScroll.getPageLeftResolved(basePi, sc.clientWidth),
      pageTop: this.virtualScroll.getPageOffset(basePi),
      pageWpx: this.virtualScroll.getPageWidth(basePi) / zoom,
      pageHpx: this.virtualScroll.getPageHeight(basePi) / zoom,
    });
  } else {
    layer.clear();
  }

  const targetH = Math.round((rawDx + snapDx) * 75); // 1 page px = 75 HWPUNIT
  const targetV = Math.round((rawDy + snapDy) * 75);
  const deltaH = targetH - state.totalDeltaH;
  const deltaV = targetV - state.totalDeltaV;
  ...
  try {
    ...
    const targets = state.multiRefs || [state.ref];
    ...
    state.lastPageX = px;
    state.lastPageY = py;
    state.totalDeltaH = targetH;
    state.totalDeltaV = targetV;
    // 연결선 자동 추적
...
export function finishPictureMoveDrag(this: any): void {
  // [캔버스 한컴 포크] 캔버스 모드: 선택된 글상자/도형을 이동 없이 재클릭 = 텍스트 편집 진입
  // (표의 pendingEnterCellHit와 대칭 — 캔바의 "선택된 텍스트 다시 클릭=편집"과 동일).
  if (this.canvasMode && this.pictureMoveState && !this.pictureMoveState.multiRefs) {
    const st = this.pictureMoveState;
    if (st.totalDeltaH === 0 && st.totalDeltaV === 0
        && st.ref?.type === 'shape' && !st.ref.cellPath && !st.ref.headerFooter) {
      const { sec, ppi, ci } = st.ref;
      ...
      this.pictureObjectRenderer?.clear();
      this.eventBus.emit('picture-object-selection-changed', false);
      this.canvasEditingRef = { kind: 'shape', sec, ppi, ci };
      this.enterTextboxEditing(sec, ppi, ci);
      this.textarea.focus();
      return;
    }
  }
  ...
  snapLayerFor(this.container).clear(); // [캔버스 한컴 포크] 정렬선 정리
}
