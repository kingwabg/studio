...
const HANDLE_ZONE_TOL_PX = 7;
const HANDLE_CURSOR: Record<string, string> = {
  nw: 'nwse-resize', se: 'nwse-resize', ne: 'nesw-resize', sw: 'nesw-resize',
  n: 'ns-resize', s: 'ns-resize', e: 'ew-resize', w: 'ew-resize',
};
function handleZoneDir(
  bbox: { x: number; y: number; width: number; height: number },
  px: number, py: number, tol: number,
): string | null {
  const x0 = bbox.x, y0 = bbox.y, x1 = bbox.x + bbox.width, y1 = bbox.y + bbox.height;
  const xm = (x0 + x1) / 2, ym = (y0 + y1) / 2;
  const n = (a: number, b: number) => Math.abs(a - b) <= tol;
  const pts: [string, number, number][] = [
    ['nw', x0, y0], ['n', xm, y0], ['ne', x1, y0],
    ['w', x0, ym], ['e', x1, ym],
    ['sw', x0, y1], ['s', xm, y1], ['se', x1, y1],
  ];
  for (const [dir, hx, hy] of pts) if (n(px, hx) && n(py, hy)) return dir;
  return null;
}
// hitTest가 표를 못 잡는 외곽선/코너에서 페이지 컨트롤 레이아웃으로 근처 표 bbox를 찾는다
// (getPageControlLayout은 셀 미열거라 getTableCellBboxes보다 가벼움 — 대형 표 hover 프리즈 회피).
function findTableBBoxNear(
  self: any, pageIdx: number, px: number, py: number, tol: number,
): { x: number; y: number; width: number; height: number } | null {
  try {
    const controls = self.wasm.getPageControlLayout(pageIdx)?.controls ?? [];
    for (const c of controls) {
      if (c.type !== 'table') continue;
      if (px >= c.x - tol && px <= c.x + c.w + tol && py >= c.y - tol && py <= c.y + c.h + tol) {
...
// [캔버스 한컴 포크] 캔버스 모드: hit가 현재 텍스트 편집 중인 개체(편집 컨텍스트)인가 —
// 편집 중인 개체 안 클릭은 개체 선택으로 가로채지 않고 일반 커서 동작으로 통과시킨다.
function isCanvasEditingHit(self: any, hit: any): boolean {
  const ref = self.canvasEditingRef;
  if (!ref || ref.kind === 'body') return false;
...
export function onClick(this: any, e: MouseEvent): void {
    ...
    if (ref && e.button === 0) {
      ...
      if (sc) {
        ...
        try {
          ...
          // [캔버스 한컴 포크] e/s/se 핸들 = 실제 크기 조절(아래·오른쪽·대각 늘리기).
          // 기존엔 핸들 위 mousedown도 이동 드래그로 소비돼 리사이즈 커서와 동작이 불일치했다.
          if (handleDir && e.button === 0 &&
              this.startTableHandleResize(handleDir, px, py, { sec: ref.sec, ppi: ref.ppi, ci: ref.ci }, pi)) {
            e.preventDefault();
            this.textarea.focus();
            return;
          }
  ...
  if (e.button === 0 && this.tableResizeRenderer) {
    let resizeHit = resolveTableResizeHit(this, pageIdx, pageX, pageY);
    // [캔버스 한컴 포크] 캐시가 차가워도 캐럿이 표 안이면 1회 조회로 리사이즈 허용(한글 규약:
    // 셀 범위 선택 없이도 경계 드래그 가능). 조회 비용은 셀 선택 모드 경로(위 688행)와 동일 —
    // "모든 일반 클릭" 조회를 막던 원래 성능 가드의 취지는 유지된다(표 안 캐럿일 때만).
    if (!resizeHit) {
      const ctx = this.cursor.getCellTableContext?.();
      if (ctx) {
        try {
          ...
          this.cachedCellBboxes = bboxes;
          const pageBboxes = bboxes.filter((b: any) => b.pageIndex === pageIdx);
          if (pageBboxes.length > 0) resizeHit = { tableRef: this.cachedTableRef, bboxes, pageBboxes };
        } catch { /* bbox 조회 실패 시 리사이즈 없이 텍스트 클릭으로 진행 */ }
      }
    }
  ...
  try {
    ...
    // [캔버스 한컴 포크] 편집 컨텍스트가 아닌 곳을 누르면 방금 만든 빈 텍스트는 소멸(일원화).
    // 삭제로 컨트롤 인덱스가 밀려 hit가 표류하므로, 삭제됐으면 이 클릭은 정리로만 소비.
    if (this.canvasMode && !isCanvasEditingHit(this, hit) && this.maybeRemoveEmptyCanvasTextbox()) {
      this.canvasEditingRef = null;
      this.textarea.focus();
      return;
    }

    // [캔버스 한컴 포크] Alt+클릭만 표 전체를 객체로 선택(이동/리사이즈용). 표는 '글 넣는 그릇'이라
    // 일반 클릭은 아래 '일반 클릭' 분기의 셀 편집(글영역)으로 두어야 한다 — 캔버스 모드 자동 잡기 제거.
    if (e.altKey && hit.parentParaIndex !== undefined && hit.controlIndex !== undefined && !hit.isTextBox) {
      this.canvasEditingRef = null; // 다른 개체를 잡으면 기존 편집 종료
      this.cursor.moveTo(hit); // 유효한 셀 위치 확보 (Esc 해제 시 커서가 표 안에 남게)
      selectTableObject.call(this, { sec: hit.sectionIndex, ppi: hit.parentParaIndex, ci: hit.controlIndex });
      return;
    }
    ...
    if (this.cursor.isInPictureObjectSelection() && hit.isTextBox
      ...
      if (ref && ref.type === 'shape'
        ...
        // [캔버스 한컴 포크] 캔버스 모드 편집 컨텍스트 기록 (재클릭=편집 규칙)
        this.canvasEditingRef = { kind: 'shape', sec: ref.sec, ppi: ref.ppi, ci: ref.ci };
    ...
    // [캔버스 한컴 포크] 캔버스 모드: 글상자 내부 클릭도 개체 선택 (경계선일 필요 없음).
    // 편집 컨텍스트(재클릭으로 진입한 글상자)는 통과시켜 캐럿 클릭 유지.
    if (this.canvasMode && hit.isTextBox
        && hit.parentParaIndex !== undefined && hit.controlIndex !== undefined
        && !isCanvasEditingHit(this, hit)) {
      // (빈 새 텍스트 정리는 위 상단에서 이미 일원화 처리)
      this.canvasEditingRef = null;
      this.cursor.clearSelection();
      this.exitPictureObjectSelectionIfNeeded();
      this.cursor.enterPictureObjectSelectionDirect(
    ...
    // ⚠ 표 셀(parentParaIndex 있음)·글상자(isTextBox)는 여기서 가로채지 않고 아래 일반 클릭으로
    ...
    if (
      ...
      // [캔버스 한컴 포크] 캔버스 모드: 표 셀 클릭 = 그 표의 글영역 편집 컨텍스트(타이핑 허용).
      if (this.canvasMode) {
        this.canvasEditingRef = { kind: 'table', sec: hit.sectionIndex, ppi: hit.parentParaIndex, ci: hit.controlIndex };
      }
      const cellRC = this.hitTestCellRowCol(e);
...
export function onDblClick(this: any, e: MouseEvent): void {
  ...
  // [캔버스 한컴 포크] 캔버스 모드: 빈 지면 더블클릭 = 그 자리에 새 텍스트(글상자) + 바로 편집
  // (캔바 텍스트 도구 — 이전 /studio insertTextAt 이식. 본문 흐름 편집은 문서 모드에서.)
  if (this.canvasMode) {
    try {
      const hit = this.hitTestFromClientPoint?.(e.clientX, e.clientY);
      if (hit && hit.parentParaIndex === undefined && !hit.isTextBox
          && hit.paragraphIndex < 0xFFFFFF00) {
        e.preventDefault();
        this.createCanvasTextboxAt(e.clientX, e.clientY, {
          sectionIndex: hit.sectionIndex,
          paragraphIndex: hit.paragraphIndex,
          charOffset: hit.charOffset ?? 0,
        });
        return;
      }
    } catch { /* hitTest 실패 시 무시 */ }
...
export function onContextMenu(this: any, e: MouseEvent): void {
  ...
  // [캔버스 한컴 포크] 글상자 안(편집 중) 우클릭 → AI 대화형 수정 진입점 (1-depth 글상자만)
  if (this.cursor.isInTextBox() && (this.cursor.getPosition().cellPath?.length ?? 0) <= 1) {
    items = [
...
export function onMouseMove(this: any, e: MouseEvent): void {
  ...
  // [캔버스 한컴 포크] 표 개체 핸들 리사이즈 드래그 중: 가이드 프리뷰 갱신
  ...
  if (this.cursor.isInTableObjectSelection() && this.tableObjectRenderer) {
    ...
    const y = e.clientY - contentRect.top;
    const ref = this.cursor.getSelectedTableRef();
    const hoverLayer = tableHoverFor(this.container);
    ...
    if (dir) {
      ...
      if (ref) {
        try {
          const bbox = this.wasm.getTableBBox(ref.sec, ref.ppi, ref.ci);
          const zoom = this.viewportManager.getZoom();
          hoverLayer.show(bbox, {
            zoom,
            pageLeft: this.virtualScroll.getPageLeftResolved(bbox.pageIndex ?? 0, scrollContent.clientWidth),
            pageTop: this.virtualScroll.getPageOffset(bbox.pageIndex ?? 0),
          });
        } catch { /* bbox 조회 실패 시 강조 없이 커서만 */ }
      }
      return;
    }

    hoverLayer.clear(); // 핸들 밖 → 전체 강조 끔
    if (ref) {
      const zoom = this.viewportManager.getZoom();
      const pi = this.virtualScroll.getPageAtPoint(x, y);
      const po = this.virtualScroll.getPageOffset(pi);
...
export function handleResizeHover(this: any, e: MouseEvent): void {
  ...
  const zoneTol = HANDLE_ZONE_TOL_PX / (zoom || 1);
  if (!tableRef) {
    // [캔버스 한컴 포크] 코너/외곽선은 hitTest가 표를 못 잡는다 → 페이지 컨트롤로 근처 표를 찾아
    // 8핸들 구역이면 "전체 표 잡기" hover(표 선택 안 해도). 그 외에는 기존 경계선 로직.
    const nearTable = findTableBBoxNear(this, pageIdx, pageX, pageY, zoneTol);
    if (nearTable) {
      const dir = handleZoneDir(nearTable, pageX, pageY, zoneTol);
      if (dir) {
        this.container.style.cursor = HANDLE_CURSOR[dir] ?? '';
        this.tableResizeRenderer.clear();
  ...
  // [캔버스 한컴 포크] 표 위(셀) hover 중에도 8핸들 구역이면 전체 표 잡기 우선(작은 표는 셀에서도 변 중점에 닿음)
  try {
  ...
  } catch { /* bbox 실패 시 경계선 로직으로 */ }
  tableHoverFor(this.container).clear();

  // 셀 bbox 캐싱 — [캔버스 한컴 포크] 선택 없이도 경계선 hover가 되도록, 다른 표로 처음 진입할 때
  // 셀 bbox를 1회 워밍한다(프레임마다 아님 — 같은 표 hover 중엔 재사용). 프레임마다 새로 만들면
  // 대형 표가 커서 이동만으로 멈추므로, "진입 1회"로 제한해 그 프리즈를 피한다.
  if (!this.cachedTableRef ||
    ...
    try {
      ...
      this.cachedTableRef = { sec: tableRef.sec, ppi: tableRef.ppi, ci: tableRef.ci };
    } catch {
      this.tableResizeRenderer.clear();
      hideProtectedCellHover(this);
      this.container.style.cursor = '';
...
export function onMouseUp(this: any, _e: MouseEvent): void {
  ...
  // [캔버스 한컴 포크] 표 개체 핸들 리사이즈 종료 — 마지막 행/열 성장 적용
