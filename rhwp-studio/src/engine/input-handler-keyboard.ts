...
export function pastePlainText(this: any, text: string, hasSelection: boolean): void {
  if (hasSelection) {
...
export function onKeyDown(this: any, e: KeyboardEvent): void {
  ...
  if (this.canvasMode && !this.canvasEditingRef
      ...
      && !this.cursor.isInHeaderFooter() && !this.cursor.isInFootnote()
  ...
  if (this.cursor.isInCellSelectionMode()) {
    ...
    // [캔버스 한컴 포크] DEL/Backspace: 행/열 전체면 삭제 모달, 그 외 내용 지우기 3지선다(한글 동작)
    if (e.key === 'Delete' || e.key === 'Backspace') {
      e.preventDefault();
      this.handleCellSelectionDelete();
      return;
    }
    // M: 셀 합치기, S: 셀 나누기
    if (e.key === 'm' || e.key === 'M') {
  ...
  if (e.key === 'Escape') {
    e.preventDefault();
    // [캔버스 한컴 포크] 편집 → 개체 선택으로 승격되므로 편집 컨텍스트는 여기서 끝난다.
    const wasCanvasEditing = this.canvasMode && !!this.canvasEditingRef;
    this.canvasEditingRef = null;
    // 더블클릭으로 만든 글상자가 빈 채면 승격 대신 소멸 (캔바: 빈 텍스트는 사라진다)
    if (this.maybeRemoveEmptyCanvasTextbox?.()) {
      this.selectionRenderer.clear();
      return;
    }
    ...
    } else if (wasCanvasEditing) {
      // [캔버스 한컴 포크] 본문 프레임 편집 → Esc = 편집 종료 (캐럿 숨김)
      this.cursor.clearSelection();
      this.caret.hide();
      this.selectionRenderer.clear();
    }
